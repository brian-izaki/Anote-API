...
module.exports = {
  index: async (req, res) => {
    ...
    try {
      ...
      return res.status(404).json({ msg: 'Anotações não encontradas' });
    } catch (error) {
  ...
  delete: async (req, res) => {
    ...
    try {
      if (!note_id) { return res.status(412).json({ msg: 'Não foi passado o id da anotação' }); }

      const hasNote = await Notes.findByPk(note_id);
      if (!hasNote) { return res.status(404).json({ msg: 'Não foi encontrada a anotação que deseja deletar' }); }
